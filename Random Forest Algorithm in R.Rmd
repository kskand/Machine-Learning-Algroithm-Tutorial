...
Deision tree builds classification models in the form of a tree structure. It breaks down a dataset into smaller subsets.
...
Random Forest is considered to be the solution of all data science problems. 
...
mydata <- iris
str(mydata)
# data distribution using sample
set.seed(123)
id<-sample(2,nrow(mydata),prob = c(0.7,0.3),replace = T)
train_dt<-mydata[id==1,]
test_dt<-mydata[id==2,]
#selection of optimum mtr
mtr<-tuneRF(train_dt,train_dt$Species,stepFactor = 1.2,improve = 0.01,trace=T,plot = T)
train_dt$Species<-as.factor(train_dt$Species)
#applying random forest
rf<-randomForest(Species~.,data = train_dt, importance=T)
#check for OOB estimate of error and confusion matrix
rf
#check Gini Index(priority of variables)
importance(rf)
varImpPlot(rf)
#use test model using test data
pred_mydata<-predict(rf,newdata = test_dt,type = 'class')
pred_mydata
#summarizing the test result using confusion matrix
confusionMatrix(table(pred_mydata,test_dt$Species))
